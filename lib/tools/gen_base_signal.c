  ...
  while ((opt = getopt(argc, argv, "cfpndvsxmo")) != -1)
    ...
    switch (opt)
      ...
      case 'd':
        ...
        break;
      case 'o':
        if ((outf = open(argv[optind], O_RDWR | O_CREAT | O_TRUNC, S_IWUSR | S_IRUSR | S_IRGRP |  S_IROTH)) == -1)
          {
          perror(argv[optind]);
          exit(-2);
          }
        if ((outd = fdopen(outf, "w+")) == NULL)
          {
          perror(argv[optind]);
          exit(-2);
          }
...
int work_enb(srslte_enb_dl_t *enb_dl,
  ...
  if (srslte_enb_dl_put_pdsch(enb_dl,
                              ...
                              softbuffer_tx,
                              rnti,
                              (int[SRSLTE_MAX_CODEWORDS])
                                {
                                dci->rv_idx, dci->rv_idx_1
                                },
                              sf_idx % 10,
                              data_tx,
                              mimo_type) < 0)
...
int main(int argc, char **argv)
  ...
  for (uint32_t sf_idx = 0; sf_idx < nof_subframes; sf_idx++)
    ...
    for (int i = 0; i < enb_dl.cell.nof_ports; i++)
      {
      if (fwrite(enb_dl.ifft[i].out_buffer, sizeof(cf_t), SRSLTE_SF_LEN_PRB(enb_dl.cell.nof_prb), outd) != SRSLTE_MAX_TB)
        {
        perror("Write");
        exit(-2);
        }
