...
void usage(char *prog)
  {
  printf("Usage: %s [agnv] -b band -e EARFCN\n", prog);
...
void parse_args(int argc, char **argv)
  {
  int opt;
  while ((opt = getopt(argc, argv, "agenvb")) != -1)
    {
    switch(opt)
      {
      case 'a':
...
//------------------------------------------------------------------------------
int rf_recv_wrapper(void *h,
                    cf_t *data[SRSLTE_MAX_PORTS],
                    uint32_t nsamples,
                    srslte_timestamp_t *t)
  {
  ...
  return srslte_rf_recv_with_time_multi((srslte_rf_t*) h,
                                        ptr,
                                        nsamples,
                                        1,
                                        NULL,
...
int main(int argc, char **argv)
  ...
  nof_freqs = srslte_band_get_fd_band(band,
                                      channels,
                                      earfcn,
                                      earfcn,
                                      MAX_EARFCN);
  ...
  if (srslte_ue_cellsearch_init_multi(&cs,
  ...
  if (cell_detect_config.max_frames_pss)
    {
    srslte_ue_cellsearch_set_nof_valid_frames(&cs,
                                              cell_detect_config.nof_valid_pss_frames);
    }
  if (cell_detect_config.init_agc)
    {
    srslte_rf_info_t *rf_info = srslte_rf_get_info(&rf);
    srslte_ue_sync_start_agc(&cs.ue_sync,
                             rf_set_rx_gain_wrapper,
                             rf_info->min_rx_gain,
                             rf_info->max_rx_gain,
      ...
      for (int i = 0; i < 3; i++)
        ...
        if (found_cells[i].psr > 10.0)
          ...
          if (ret == SRSLTE_UE_MIB_FOUND)
            ...
            if (cell.nof_ports > 0)
              {
              memcpy(&results[n_found_cells].cell,
                     &cell,
