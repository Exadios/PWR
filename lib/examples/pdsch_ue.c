...
int main(int argc, char **argv)
  ...
  srslte_chest_dl_cfo_estimate_enable(&ue_dl.chest,
                                      prog_args.enable_cfo_ref,
                                      1023);
            ...
            if (n < 0)
                ...
                if(sfidx == 1)
                  ...
                  for (uint32_t tb = 0; tb < SRSLTE_MAX_CODEWORDS; tb++)
                    ...
                    if (ue_dl.pdsch_cfg.grant.tb_en[tb])
                      {
                      srslte_netsink_write(&net_sink, data[tb],
                                           1 + (ue_dl.pdsch_cfg.grant.mcs[tb].tbs - 1) / 8);
                      }
                    }
                  }
                }
              ...
              if (ue_dl.dl_dci.mcs_idx != old_dl_dci.mcs_idx           ||
                  memcmp(&ue_dl.dl_dci.type0_alloc,
                         &old_dl_dci.type0_alloc,
                         sizeof(srslte_ra_type0_t))                    ||
                  memcmp(&ue_dl.dl_dci.type1_alloc,
                         &old_dl_dci.type1_alloc,
                         sizeof(srslte_ra_type1_t))                    ||
                  memcmp(&ue_dl.dl_dci.type2_alloc,
                         &old_dl_dci.type2_alloc,
                         sizeof(srslte_ra_type2_t)))
                {
