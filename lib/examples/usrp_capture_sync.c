...
//------------------------------------------------------------------------------
int srslte_rf_recv_wrapper(void * h,
                           cf_t * data[SRSLTE_MAX_PORTS],
                           uint32_t nsamples,
                           srslte_timestamp_t * t)
  {
  DEBUG(" ----  Receive %d samples  ---- \n", nsamples);
  return srslte_rf_recv_with_time_multi(h,
                                       (void**) data,
                                       nsamples,
                                       true,
                                       NULL,
...
int main(int argc, char **argv)
  ...
  if (srslte_ue_sync_init_multi(&ue_sync,
                                cell.nof_prb,
                                cell.id == 1000,
  ...
  while((subframe_count < nof_subframes || nof_subframes == -1)
    ...
    if (!keep_running)
      {
      if (!start_capture ||
          (start_capture && srslte_ue_sync_get_sfidx(&ue_sync) == 9))
        {
        stop_capture = true;
        }
      }
