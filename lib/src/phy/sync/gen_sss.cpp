...
void generate_m0m1(uint32_t N_id_1, uint32_t *m0, uint32_t *m1)
  {
  uint32_t q_prime = N_id_1 / (LTE_SSS_N - 1);
  uint32_t q = (N_id_1 + (q_prime * (q_prime + 1) / 2)) / (LTE_SSS_N - 1);
  uint32_t m_prime = N_id_1 + (q * (q + 1) / 2);
  *m0 = m_prime % LTE_SSS_N;
  *m1 = (*m0 + m_prime / LTE_SSS_N + 1) % LTE_SSS_N;
  }
...
void generate_c(int *c, int *c_tilde, uint32_t N_id_2, bool is_c0)
  ...
  for (i = 0; i < LTE_SSS_N; i++)
    {
    c[i] = c_tilde[(i + N_id_2 + (is_c0 ? 3 : 0)) % LTE_SSS_N];
    }
  }
...
void generate_z(int *z, int *z_tilde, uint32_t m0_m1)
  ...
  for (i = 0; i < LTE_SSS_N; i++)
    {
    z[i] = z_tilde[(i + (m0_m1 % 8)) % LTE_SSS_N];
    }
  }
...
void generate_sss_all_tables(LteSssTables_t *tables, uint32_t N_id_2)
  {
  uint32_t i;
  int s_t[LTE_SSS_N], c_t[LTE_SSS_N], z_t[LTE_SSS_N];
