...
//------------------------------------------------------------------------------
void LteBitInterleaverInit(LteBitInterleaver_t *q,
                                 uint16_t *interleaver,
                                 uint32_t nof_bits)
  {
  static const uint8_t mask[] = { 0x80, 0x40, 0x20, 0x10, 0x8, 0x4, 0x2, 0x1 };

  bzero(q, sizeof(LteBitInterleaver_t));

  q->interleaver = (uint16_t *)LteVecMalloc(sizeof(uint16_t)*nof_bits);
  q->byte_idx    = (uint16_t *)LteVecMalloc(sizeof(uint16_t)*nof_bits);
  q->bit_mask    = (uint8_t *)LteVecMalloc(sizeof(uint8_t)*nof_bits);
  q->nof_bits    = nof_bits;
...
//------------------------------------------------------------------------------
void LteBitInterleaverFree(LteBitInterleaver_t *q)
  {
  if (q->interleaver)
    {
    free(q->interleaver);
...
//------------------------------------------------------------------------------
void LteBitInterleaverRun(LteBitInterleaver_t *q,
                          uint8_t *input,
                          uint8_t *output,
                          uint16_t w_offset)
  {
  ...
  if (w_offset < 8 && w_offset > 0)
    {
    st = 1;
    for (uint16_t j = 0; j < 8 - w_offset; j++)
      {
...
//------------------------------------------------------------------------------
void LteBitInterleaveI(uint8_t *input, uint8_t *output, uint32_t *interleaver, uint32_t nof_bits)
  {
  LteBitInterleaveIWOffset(input, output, interleaver, nof_bits, 0);
  }

//------------------------------------------------------------------------------
void LteBitInterleaveIWOffset(uint8_t *input, uint8_t *output, uint32_t *interleaver, uint32_t nof_bits, uint32_t w_offset)
  {
  uint32_t st = 0, w_offset_p = 0;
  static const uint8_t mask[] = { 0x80, 0x40, 0x20, 0x10, 0x8, 0x4, 0x2, 0x1 };
...
//------------------------------------------------------------------------------
void LteBitInterleave(uint8_t *input, uint8_t *output, uint16_t *interleaver, uint32_t nof_bits)
  {
  LteBitInterleaveWOffset(input, output, interleaver, nof_bits, 0);
  }

//------------------------------------------------------------------------------
void LteBitInterleaveWOffset(uint8_t *input, uint8_t *output, uint16_t *interleaver, uint32_t nof_bits, uint32_t w_offset)
  {
  uint32_t st = 0, w_offset_p = 0;
  static const uint8_t mask[] = { 0x80, 0x40, 0x20, 0x10, 0x8, 0x4, 0x2, 0x1 };
...
static void
BitarrayCopy(const unsigned char *src_org, int src_offset, int src_len,
...
//------------------------------------------------------------------------------
void LteBitCopy(uint8_t *dst, uint32_t dst_offset, uint8_t *src, uint32_t src_offset, uint32_t nof_bits)
  {
  ...
  if ((dst_offset % 8) == 0 && (src_offset % 8) == 0)
    ...
    {
    BitarrayCopy(src, src_offset, nof_bits, dst, dst_offset);
    }
  }

//------------------------------------------------------------------------------
void LteBitUnpackVector(uint8_t *packed, uint8_t *unpacked, int nof_bits)
  {
  uint32_t i, nbytes;
  nbytes = nof_bits / 8;
  for (i = 0; i < nbytes; i++)
    {
    LteBitUnpack(packed[i], &unpacked, 8);
    }
  if (nof_bits % 8)
    {
    LteBitUnpack(packed[i] >> (8 - nof_bits % 8), &unpacked, nof_bits % 8);
    }
  }

//------------------------------------------------------------------------------
void LteBitUnpackL(uint64_t value, uint8_t **bits, int nof_bits)
  {
  int i;
...
//------------------------------------------------------------------------------
void LteBitUnpack(uint32_t value, uint8_t **bits, int nof_bits)
  {
  int i;
...
//------------------------------------------------------------------------------
void LteBitPackVector(uint8_t *unpacked, uint8_t *packed, int nof_bits)
  {
  uint32_t i, nbytes;
  nbytes = nof_bits / 8;
  for (i = 0; i < nbytes; i++)
    {
    packed[i] = LteBitPack(&unpacked, 8);
    }
  if (nof_bits % 8)
    {
    packed[i] = LteBitPack(&unpacked, nof_bits % 8);
    packed[i] <<= 8 - (nof_bits % 8);
    }
  }

//------------------------------------------------------------------------------
uint32_t LteBitPack(uint8_t **bits, int nof_bits)
  {
  int i;
  uint32_t value = 0;
...
//------------------------------------------------------------------------------
uint64_t LteBitPackL(uint8_t **bits, int nof_bits)
  {
  int i;
  uint64_t value = 0;
...
//------------------------------------------------------------------------------
void LteBitFprint(FILE *stream, uint8_t *bits, int nof_bits)
  {
  int i;
...
//------------------------------------------------------------------------------
uint32_t LteBitDiff(uint8_t *x, uint8_t *y, int nbits)
  {
  uint32_t errors = 0;
...
uint32_t LteBitCount(uint32_t n)
  {
  int c;
  for (c = 0; n; c++)
    n &= n - 1;
