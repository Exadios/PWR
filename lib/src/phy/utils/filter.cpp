...
#define LTE_NUM_FILTERS 8
#define LTE_MAX_FILTER_SIZE 11

float lte_filt_decim2[LTE_NUM_FILTERS][LTE_MAX_FILTER_SIZE] =
  {
...
float lte_filt_decim3[LTE_NUM_FILTERS][LTE_MAX_FILTER_SIZE] =
  {
...
float lte_filt_decim4[LTE_NUM_FILTERS][LTE_MAX_FILTER_SIZE]  =
  {
...
void LteFiltDecimCcInit(LteFiltCc_t *q, int factor, int order)
  ...
  q->num_taps = order + 1;
  q->is_decimator = true;
  q->taps = (float *)malloc(q->num_taps * sizeof(float));

  switch(q->factor)
    {
    case 2:
      for(int i = 0; i <(q->num_taps); i++)
        q->taps[i] = lte_filt_decim2[(q->num_taps) - 4][i];
      break;
    case 3:
      for(int i = 0; i <(q->num_taps); i++)
        q->taps[i] = lte_filt_decim3[(q->num_taps) - 4][i];
      break;
    case 4:
      for(int i = 0; i <(q->num_taps); i++)
        q->taps[i] = lte_filt_decim4[(q->num_taps) - 4][i];
      break;
    default:
...
//------------------------------------------------------------------------------
void LteFiltDecimCcExecute(LteFiltCc_t *q,
                           Cf_t *input,
                           Cf_t *downsampled_input,
                           Cf_t *output, int size)
  {
  // we assume that "downsampled_input" made size (input/2 + order) so as to have prepended zeros //
  LteDownsampleCc(input, downsampled_input + (q->num_taps - 1), q->factor, size);

  for(int i = 0; i < size / q->factor; i++)
    {
    output[i] = LteVecDotProdCfc(&(downsampled_input[i]), q->taps, q->num_taps);
    }
