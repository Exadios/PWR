...
class PhchWorker : public srslte::thread_pool::worker
  {
public:
  ...
  ~PhchWorker();
  void  reset(); 
  void  set_common(PhchCommon* phy);
  void  enable_pdsch_coworker();
  bool  init(uint32_t max_prb, srslte::log* log, srslte::log* log_phy_lib_h, ChestFeedbackItf* chest_loop);
