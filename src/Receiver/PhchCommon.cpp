...
#include "srsue/hdr/phy/PhchCommon.h"
#include "asn1/rrc_asn1.h"
#include "srslte.h"
#include <assert.h>
#include <sstream>
#include <string.h>
...
PhchCommon::PhchCommon(uint32_t max_workers) : tx_sem(max_workers)
  {
  config    = NULL;
  args      = NULL;
  log_h     = NULL;
...
void PhchCommon::set_nof_workers(uint32_t nof_workers)
  {
  this->nof_workers = nof_workers;
  }

void PhchCommon::Init(phy_interface_rrc::phy_cfg_t *_config, phy_args_t *_args, srslte::log *_log, srslte::radio *_radio, rrc_interface_phy *_rrc, mac_interface_phy *_mac)
  {
  log_h     = _log;
  radio_h   = _radio;
  rrc       = _rrc;
...
bool PhchCommon::ul_rnti_active(uint32_t tti)
  {
...
bool PhchCommon::dl_rnti_active(uint32_t tti)
  {
...
srslte::radio* PhchCommon::get_radio()
  {
  return radio_h;
  }

// Unpack RAR grant as defined in Section 6.2 of 36.213
void PhchCommon::set_rar_grant(uint32_t tti, uint8_t grant_payload[SRSLTE_RAR_GRANT_LEN])
  {
...
bool PhchCommon::get_pending_rar(uint32_t tti, srslte_dci_rar_grant_t *rar_grant_)
  {
...
uint16_t PhchCommon::get_ul_rnti(uint32_t tti)
  {
...
srslte_rnti_type_t PhchCommon::get_ul_rnti_type()
  {
  return ul_rnti_type;
  }
void PhchCommon::set_ul_rnti(srslte_rnti_type_t type, uint16_t rnti_value, int tti_start, int tti_end)
  {
  ul_rnti = rnti_value;
  ul_rnti_type = type;
...
uint16_t PhchCommon::get_dl_rnti(uint32_t tti)
  {
...
srslte_rnti_type_t PhchCommon::get_dl_rnti_type()
  {
  return dl_rnti_type;
  }
void PhchCommon::set_dl_rnti(srslte_rnti_type_t type, uint16_t rnti_value, int tti_start, int tti_end)
  {
  dl_rnti       = rnti_value;
  dl_rnti_type  = type;
...
void PhchCommon::reset_pending_ack(uint32_t tti)
  {
  pending_ack[TTIMOD(tti)].enabled = false;
  }

void PhchCommon::set_pending_ack(uint32_t tti, uint32_t I_lowest, uint32_t n_dmrs)
  {
...
bool PhchCommon::get_pending_ack(uint32_t tti)
  {
  return get_pending_ack(tti, NULL, NULL);
  }

bool PhchCommon::get_pending_ack(uint32_t tti, uint32_t *I_lowest, uint32_t *n_dmrs)
  {
  if (I_lowest)
    {
...
bool PhchCommon::is_any_pending_ack()
  {
 ...
 */
void PhchCommon::worker_end(uint32_t tti, bool tx_enable,
...
void PhchCommon::set_cell(const srslte_cell_t &c)
  {
  cell = c;
  }

uint32_t PhchCommon::get_nof_prb()
  {
  return cell.nof_prb;
  }

void PhchCommon::set_dl_metrics(const dl_metrics_t &m)
  {
  if(dl_metrics_read)
    {
    dl_metrics       = m;
...
void PhchCommon::get_dl_metrics(dl_metrics_t &m)
  {
  m = dl_metrics;
  dl_metrics_read = true;
  }

void PhchCommon::set_ul_metrics(const ul_metrics_t &m)
  {
  if(ul_metrics_read)
    {
    ul_metrics       = m;
...
void PhchCommon::get_ul_metrics(ul_metrics_t &m)
  {
  m = ul_metrics;
  ul_metrics_read = true;
  }

void PhchCommon::set_sync_metrics(const sync_metrics_t &m)
  {
...
void PhchCommon::get_sync_metrics(sync_metrics_t &m)
  {
  m = sync_metrics;
  sync_metrics_read = true;
  }

void PhchCommon::reset()
  {
  sr_enabled        = false;
  is_first_of_burst = true;
  is_first_tx       = true;
...
*/
void PhchCommon::build_mch_table()
  {
  // First reset tables
...
void PhchCommon::build_mcch_table()
  {
  // First reset tables
...
void PhchCommon::set_mcch()
  {
  mcch_configured = true;
  }

void PhchCommon::set_mch_period_stop(uint32_t stop)
  {
...
bool PhchCommon::is_mch_subframe(subframe_cfg_t* cfg, uint32_t phy_tti)
  {
...
bool PhchCommon::is_mcch_subframe(subframe_cfg_t* cfg, uint32_t phy_tti)
  {
  uint32_t sfn; // System Frame Number
  uint8_t  sf;  // Subframe
  uint8_t  offset;
...
void PhchCommon::get_sf_config(subframe_cfg_t *cfg, uint32_t phy_tti)
  {
