...
the target be measured and, if so, how{\textquotedblright}? Measuring the
angle imposes difficulties but yields an extra degree of freedom so that
it will be possible to get target estimates from just one transmitter.
However, it may be necessary to drop this requirement and fall back on a
time difference or pseudo time zero scheme.}}
